  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (USE_SUPABASE) {
        ...
        // OAuth redirect will handle the rest - don't set loading to false
      } else {
        // Fallback to mock
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="space-y-4">
          <Button
            ...
            disabled={isLoading}
            size="lg"
            className="w-full h-14 text-lg font-semibold bg-white hover:bg-gray-50 text-gray-900 border-2 border-gray-200 rounded-2xl transition-all shadow-sm"
          >
            {isLoading ? (
        ...
        <div className="mt-6 p-4 rounded-2xl bg-secondary/50 border border-border">
          <p className="text-sm text-muted-foreground text-center">
            Sign in with your Google account to access ParkEase and start booking parking spots.
          </p>
        </div>
      </div>
